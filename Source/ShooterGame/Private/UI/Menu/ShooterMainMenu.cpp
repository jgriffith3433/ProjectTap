...
static const FString MapNames[] = { TEXT("AlienLab/Lab_Deathmatch"), TEXT("AlienCity/City_Deathmatch"), TEXT("AlienShip/Ship_Deathmatch") };
static const FString JoinMapNames[] = { TEXT("Any"), TEXT("Lab_Deathmatch"), TEXT("City_Deathmatch"), TEXT("Ship_Deathmatch") };
static const FName PackageNames[] = { TEXT("Lab_Deathmatch.umap"), TEXT("City_Deathmatch.umap"), TEXT("Ship_Deathmatch.umap") };
...
{	
#if SHOOTER_CONSOLE_UI
	// Menu was hidden from the top-level main menu, on consoles show the login screen again.
	if ( ensure(GameInstance.IsValid()))
	{
		GameInstance->GotoState(ShooterGameInstanceState::LoginScreen);
	}
#else
...
FShooterMainMenu::EMap FShooterMainMenu::GetSelectedMap() const
{
	...
	return EMap::ELab_Deathmatch;	// Need to return something (we can hit this path in cooking)
}
