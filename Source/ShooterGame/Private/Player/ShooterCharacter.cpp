...
float AShooterCharacter::TakeDamage(float Damage, struct FDamageEvent const& DamageEvent, class AController* EventInstigator, class AActor* DamageCauser)
{
	...
	if (EventInstigator)
	{
		if (this != EventInstigator->GetPawn())
		{
...
void AShooterCharacter::OnDeathDestroy()
{
...
void AShooterCharacter::OnDeath(float KillingDamage, struct FDamageEvent const& DamageEvent, class APawn* PawnInstigator, class AActor* DamageCauser)
{
	...
	GetWorldTimerManager().SetTimer(TimerHandle_DeathDestroy, this, &AShooterCharacter::OnDeathDestroy, 2.0f, false);
	...
	// Apply some velocity up so we don't go through the floor
	UShooterCharacterMovement* characterMovementComp = Cast<UShooterCharacterMovement>(GetCharacterMovement());
	if (characterMovementComp != NULL)
	{
		characterMovementComp->AddImpulse(FVector::UpVector * 500.0f, true);
	}
...
void AShooterCharacter::PlayHit(float DamageTaken, struct FDamageEvent const& DamageEvent, class APawn* PawnInstigator, class AActor* DamageCauser)
{
	...
	if (DamageType && DamageType->bKnockPlayerBack)
	{
	...
	if (PawnInstigator && PawnInstigator->IsLocallyControlled())
	{
...
void AShooterCharacter::ApplyDamageMomentum(float DamageTaken, FDamageEvent const& DamageEvent, APawn* PawnInstigator, AActor* DamageCauser)
{
	//UShooterDamageType const* const DmgTypeCDO = DamageEvent.DamageTypeClass->GetDefaultObject<UShooterDamageType>();
	...
	if (characterMovementComp != nullptr)
	{
		...
		if (DamageEvent.IsOfType(FRadialDamageEvent::ClassID))
		{
			...
			MyRadialDamageEvent->GetBestHitInfo(this, PawnInstigator, HitInfo, ImpulseDir);
		}
		else if (DamageEvent.IsOfType(FPointDamageEvent::ClassID))
		{
			const FPointDamageEvent* MyPointDamageEvent = (FPointDamageEvent*)&DamageEvent;
			MyPointDamageEvent->GetBestHitInfo(this, PawnInstigator, HitInfo, ImpulseDir);
		}
		FVector Impulse = ImpulseDir * DmgTypeCDO->DamageImpulse;
		bool const bMassIndependentImpulse = !DmgTypeCDO->bScaleMomentumByMass;
		characterMovementComp->StopMovementImmediately();
		characterMovementComp->AddImpulse(Impulse, bMassIndependentImpulse);
	}
}
...
void AShooterCharacter::SetCurrentWeapon(AShooterWeapon* NewWeapon, AShooterWeapon* LastWeapon)
{
	...
	if (NewWeapon)
	{
		...
		NewWeapon->OnEquip(LastWeapon, bWantsToFire);
	}
...
void AShooterCharacter::SetTargeting(bool bNewTargeting)
{
	...
	if (bNewTargeting)
	{
		AShooterWeapon* NextWeapon = Inventory[(1) % Inventory.Num()];
		EquipWeapon(NextWeapon);
	}
	else
	{
		AShooterWeapon* NextWeapon = Inventory[(0) % Inventory.Num()];
		EquipWeapon(NextWeapon);
	}
